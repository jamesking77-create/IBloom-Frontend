...
const OrderPreviewStep = ({
  ...
  // The two WhatsApp numbers to choose between
  ...
  // Submits the order to the backend. Used to be behind its own "Confirm Order"
  // button; now runs as part of the WhatsApp send below (see sendOrderToWhatsAppNumber).
  const submitOrderToBackend = useCallback(async () => {
  ...
  // Builds a full order summary as readable WhatsApp text — mirrors what
  // submitOrderToBackend sends, but formatted for a chat instead of an API payload.
  const buildOrderWhatsAppMessage = useCallback(() => {
...
*Customer*
Name: ${customerInfo?.name || "Not specified"}
...
Duration: ${orderDuration} day${orderDuration === 1 ? "" : "s"}

*Delivery/Setup*
Delivery: ${customerInfo?.delivery === "yes" ? `Yes - Deliver to ${customerInfo?.location || "location"}` : "No - Warehouse pickup"}
Setup: ${customerInfo?.installation === "yes" ? "Yes - Professional setup" : "No - Self setup"}

${customerInfo?.specialRequests ? `*Special Requests*\n${customerInfo.specialRequests}\n\n` : ""}*Items (${calculations.itemCount})*

${itemLines || "No items selected"}

*Pricing*
Items Subtotal: ${formatPrice(calculations.subtotal)}
Tax (7.5%): ${formatPrice(calculations.tax)}
*Items + Tax: ${formatPrice(calculations.total)}*
(Delivery/setup fees and any refundable deposit not included — to be confirmed here)`;
  }, [
    cartItems,
    customerInfo,
    selectedDates,
  ...
  // Confirms the order (submits to the backend) and opens WhatsApp with the full
  // summary for the chosen sales line. WhatsApp still opens even if the backend
  // submission has an issue, since that's the client's actual lead channel.
  const sendOrderToWhatsAppNumber = useCallback(
    async (phone) => {
    ...
    [submitOrderToBackend, buildOrderWhatsAppMessage],
  );

  const handleShareOrderToWhatsApp = useCallback(() => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fadeIn px-2 sm:px-0">
      ...
      <div className="bg-bloom-green-50 rounded-2xl sm:rounded-3xl shadow-xl p-4 sm:p-8 border border-white/20">
        ...
        {customerInfo?.specialRequests && (
          <div className="mt-4 sm:mt-6 bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-white/20">
            <div className="flex items-center mb-3 sm:mb-4">
              <MessagesSquare className="w-5 h-5 sm:w-6 sm:h-6 text-bloom-rose-500 mr-2 sm:mr-3" />
              <h3 className="font-semibold text-gray-800 text-sm sm:text-base">
                Special Requests
              </h3>
            </div>
      ...
      {/*
        There used to be a separate "Confirm Order" button here. It's been removed:
        delivery/setup fees depend on distance and setup complexity, so we can't
        show a trustworthy final total on-site, and pricing gets finalized in the
        WhatsApp chat anyway. The WhatsApp button below now does both jobs — it
        submits the order AND sends the summary.
      */}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center pt-4 sm:pt-6 gap-3">
        ...
        <div className="relative w-full sm:flex-1 sm:max-w-md">
            ...
            <>
              ...
              <div className="absolute bottom-full mb-2 left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden z-20">
                ...
                {whatsappNumbers.map((num) => (
                  <button
                    key={num.phone}
                    onClick={() => sendOrderToWhatsAppNumber(num.phone)}
                    type="button"
          ...
          <button
            onClick={handleShareOrderToWhatsApp}
            disabled={
